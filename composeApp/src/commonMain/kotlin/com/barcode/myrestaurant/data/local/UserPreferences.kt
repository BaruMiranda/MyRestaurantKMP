...
class UserPreferences(private val settings: Settings) {
    ...
    companion object {
        ...
        const val KEY_TYPE_APP        = "type_app"
    }
    ...
    fun clearSession() {
        listOf(
            KEY_ACCESS_TOKEN, KEY_REFRESH_TOKEN, KEY_SESSION_ID,
            KEY_USER_ID, KEY_FIRST_NAME, KEY_LAST_NAME, KEY_EMAIL,
            KEY_PHONE, KEY_PHONE_COUNTRY, KEY_AVATAR_URL, KEY_STATUS,
            KEY_LOCALE, KEY_TIMEZONE, KEY_LAST_LOGIN_AT, KEY_CREATED_AT,
            KEY_TOKEN_TYPE, KEY_EXPIRES_IN, KEY_REFRESH_EXPIRES,
            "EMAIL", "NAME", "PHONE",
        ).forEach { settings.remove(it) }
        settings.remove(KEY_EMAIL_VERIFIED)
        settings.remove(KEY_PHONE_VERIFIED)
    }

    fun setOnboardingDone(typeApp: String) {
        settings.putBoolean(KEY_ONBOARDING_DONE, true)
        settings.putString(KEY_TYPE_APP, typeApp)
    }

    fun isOnboardingDone(): Boolean = settings.getBoolean(KEY_ONBOARDING_DONE, false)

    fun getTypeApp(): String = settings.getString(KEY_TYPE_APP, "persona")
}
